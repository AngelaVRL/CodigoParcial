...
const dateSInput = document.getElementById("start_date");
const dateEInput = document.getElementById("end_date");
const nameHnput = document.getElementById("reservation_holder");
const nameGnput = document.getElementById("group");
...
getOneButton.addEventListener('click', async function () {
    ...
    try {
        const response = await fetch(`/heritage/${id}`);
        const data = await response.json();
